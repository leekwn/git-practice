...
def main():
    parser = argparse.ArgumentParser(description="Filter out short or ambiguous influenza sequences and align them.")
    parser.add_argument("--indir", required=True, help="Directory containing per-segment FASTA files")
    parser.add_argument("--outdir", default="filtered_segments", help="Output directory for filtered FASTAs")
    parser.add_argument("--aligndir", default="aligned_segments", help="Output directory for aligned FASTAs (MAFFT)")
    ...
    parser.add_argument("--mafft_cmd", default="mafft",
                        help="Command or full path to MAFFT executable (default: 'mafft')")
    args = parser.parse_args()

    os.makedirs(args.outdir, exist_ok=True)
    os.makedirs(args.aligndir, exist_ok=True)
    ...
    for fname in os.listdir(args.indir):
        ...
        filtered_path = os.path.join(args.outdir, fname)
        ...
        # 2) Align with MAFFT
        aligned_path = os.path.join(args.aligndir, fname.replace(".fasta", "_aln.fasta"))

        # Skip empty filtered files
        if os.path.getsize(filtered_path) == 0:
            print(f"{fname}: filtered file is empty, skipping MAFFT.")
            continue

        print(f"{fname}: running MAFFT alignment...")
        subprocess.run(
            [args.mafft_cmd, "--auto", filtered_path],
            stdout=open(aligned_path, "w"),
            check=True
        )
        print(f"{fname}: alignment written to {aligned_path}")
